# @f1stnpm3/neque-culpa-voluptates
[![license](https://img.shields.io/badge/license-MIT-brightgreen)](LICENSE)
[![npm version](https://img.shields.io/npm/v/@f1stnpm3/neque-culpa-voluptates.svg?style=flat-square)](https://www.npmjs.org/package/@f1stnpm3/neque-culpa-voluptates)
...
```JavaScript
import { EncoderStream } from '@f1stnpm3/neque-culpa-voluptates';
...
```JavaScript
import { encodeAsIterable } from '@f1stnpm3/neque-culpa-voluptates';
...
```JavaScript
import { encodeAsAsyncIterable } from '@f1stnpm3/neque-culpa-voluptates';
...
```HTML
<script src="node_modules/@f1stnpm3/neque-culpa-voluptates/dist/index.js"></script>
```
...
```JavaScript
import { Encoder } from '@f1stnpm3/neque-culpa-voluptates';
let encoder = new Encoder();
encoder.encode(myBigData);
```

Another way to further leverage the benefits of the @f1stnpm3/neque-culpa-voluptates record structures is to use streams that naturally allow for data to reuse based on previous record structures. The stream classes have the record structure extension enabled by default and provide excellent out-of-the-box performance.
...
Another useful way of using @f1stnpm3/neque-culpa-voluptates, and the record extension, is for storing data in a databases, files, or other storage systems. If a number of objects with common data structures are being stored, a shared structure can be used to greatly improve data storage and deserialization efficiency. In the simplest form, provide a `structures` array, which is updated if any new object structure is encountered:
...
* `useFloat32` - This will enable @f1stnpm3/neque-culpa-voluptates to encode non-integer numbers as 32-bit (4 byte) floating point numbers. See next section for possible values.
* `alwaysUseFloat` - This will force @f1stnpm3/neque-culpa-voluptates to encode any number, including integers, as floating-point numbers.
...
```JavaScript
import { ALWAYS, DECIMAL_ROUND, DECIMAL_FIT } from '@f1stnpm3/neque-culpa-voluptates'
```
...
require("@f1stnpm3/neque-culpa-voluptates").encode(obj);                             |  163100 |  5001 |  32613
require("@f1stnpm3/neque-culpa-voluptates").decode(buf);                             |  100200 |  5004 |  20023
@f1stnpm3/neque-culpa-voluptates w/ shared structures: packr.encode(obj);            |  178300 |  5002 |  35645
@f1stnpm3/neque-culpa-voluptates w/ shared structures: packr.decode(buf);            |  414000 |  5000 |  82800
...
(`avsc` is schema-based and more comparable in style to @f1stnpm3/neque-culpa-voluptates with shared structures).

Here is a benchmark of streaming data (again borrowed from `msgpack-lite`'s benchmarking), where @f1stnpm3/neque-culpa-voluptates is able to take advantage of the structured record extension and really demonstrate its performance capabilities:
...
## Custom Extensions
You can add your own custom extensions, which can be used to encode specific types/classes in certain ways. This is done by using the `addExtension` function, and specifying the class, extension type code (custom extensions should be a number greater than 40500, all others are reserved for  CBOR or @f1stnpm3/neque-culpa-voluptates), and your encode and decode functions (or just the one you need). You can use @f1stnpm3/neque-culpa-voluptates encoding and decoding within your extensions:
```JavaScript
import { addExtension, Encoder } from '@f1stnpm3/neque-culpa-voluptates';
...
### Structured Cloning
With structured cloning enabled, @f1stnpm3/neque-culpa-voluptates will also use tags/extensions to store Set, Map, Error, RegExp, ArrayBufferView objects and preserve their types.
